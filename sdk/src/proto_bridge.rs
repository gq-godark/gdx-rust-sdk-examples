...
/// Validate spline region layout before sending to the edge.
pub fn validate_spline_regions(
    bid_regions: &[crate::types::SplineRegionInput],
    ask_regions: &[crate::types::SplineRegionInput],
) -> Result<(), GodarkError> {
    if bid_regions.is_empty() && ask_regions.is_empty() {
        return Err(GodarkError::InvalidInput(
            "at least one of bid_regions or ask_regions must be non-empty".into(),
        ));
    }
    validate_spline_side_regions("bid_regions", bid_regions)?;
    validate_spline_side_regions("ask_regions", ask_regions)?;
    Ok(())
}

fn validate_spline_side_regions(
    name: &str,
    regions: &[crate::types::SplineRegionInput],
) -> Result<(), GodarkError> {
    if regions.len() > MAX_SPLINE_REGIONS_PER_SIDE {
        return Err(GodarkError::InvalidInput(format!(
            "{name}: too many regions ({}, max {MAX_SPLINE_REGIONS_PER_SIDE})",
            regions.len()
        )));
    }
    ...
    for (idx, region) in regions.iter().enumerate() {
        if region.start_offset == 0 {
            return Err(GodarkError::InvalidInput(format!(
                "{name}[{idx}]: start_offset must be > 0"
            )));
        }
        if region.start_offset >= region.end_offset {
            return Err(GodarkError::InvalidInput(format!(
                "{name}[{idx}]: start_offset must be < end_offset"
            )));
        }
        ...
        if let Some(prev) = prev_end {
            if region.start_offset < prev {
                return Err(GodarkError::InvalidInput(format!(
                    "{name}[{idx}]: regions must be sorted and non-overlapping"
                )));
            }
        }
...
fn validate_anchor_price(anchor_price: f64) -> Result<(), GodarkError> {
    if !anchor_price.is_finite() || anchor_price <= 0.0 {
        return Err(GodarkError::InvalidInput(
            "anchor_price must be finite and > 0".into(),
        ));
    }
    Ok(())
}

/// Build a `SplineOrderInput` wrapped in an `EdgeSequencerRequest`.
pub fn build_place_spline_order_proto(
    symbol_id: u64,
    user_uuid: &[u8],
    anchor_price: f64,
    bid_regions: &[crate::types::SplineRegionInput],
    ...
    validate_anchor_price(anchor_price)?;
    validate_spline_regions(bid_regions, ask_regions)?;
    let spline = sequencer::SplineOrderInput {
        symbol_id,
        user_commitment: Vec::new(),
        anchor_price,
        bid_regions: bid_regions.iter().map(spline_region_to_proto).collect(),
        ask_regions: ask_regions.iter().map(spline_region_to_proto).collect(),
        correlation_id: correlation_id_body_bytes(correlation_id_bytes),
        user_uuid: user_uuid.to_vec(),
        leverage,
        stp_mode,
    ...
    let req = sequencer::EdgeSequencerRequest {
        inner: Some(sequencer::edge_sequencer_request::Inner::SplineOrder(
            spline,
        )),
    };
...
/// Build a `SplineAnchorUpdateInput` wrapped in an `EdgeSequencerRequest`.
pub fn build_spline_anchor_update_proto(
    order_id: u64,
    symbol_id: u64,
    user_uuid: &[u8],
    anchor_price: f64,
    ...
    validate_anchor_price(anchor_price)?;
    let update = sequencer::SplineAnchorUpdateInput {
        order_id,
        user_commitment: Vec::new(),
        symbol_id,
        anchor_price,
    ...
    let req = sequencer::EdgeSequencerRequest {
        inner: Some(sequencer::edge_sequencer_request::Inner::SplineAnchorUpdate(
            update,
        )),
    };
    Ok(req.encode_to_vec())
...
/// Decode a `NodeResponse` carrying a `SplineOrderAck`.
pub fn parse_spline_order_ack(data: &[u8]) -> Result<crate::types::SplineOrderAck, GodarkError> {
    let resp = sequencer::NodeResponse::decode(data)
        .map_err(|e| GodarkError::Encryption(format!("decode NodeResponse: {e}")))?;
    let ack = match resp.inner {
        Some(sequencer::node_response::Inner::SplineOrderAck(a)) => a,
        other => {
            return Err(GodarkError::Order {
                message: format!(
...
pub fn parse_node_response(data: &[u8]) -> Result<NodeResponseKind, GodarkError> {
    ...
    match resp.inner {
        ...
        | Some(sequencer::node_response::Inner::SplineOrderAck(_)) => Ok(NodeResponseKind::Unknown),
...
mod tests {
    ...
    fn test_build_place_spline_order_proto_roundtrip() {
        ...
        assert_eq!(spline.symbol_id, 7);
        assert_eq!(spline.anchor_price, 50_000.0);
        assert_eq!(spline.leverage, 2);
        assert_eq!(spline.bid_regions.len(), 1);
        assert_eq!(spline.ask_regions.len(), 1);
        assert_eq!(spline.bid_regions[0].start_offset, 1);
        assert_eq!(spline.bid_regions[0].end_offset, 4);
        assert_eq!(spline.bid_regions[0].density, 0.5);
    }

    #[test]
    fn test_build_spline_anchor_update_proto_roundtrip() {
        let bytes = build_spline_anchor_update_proto(42, 7, &TEST_UUID, 51_000.0, &TEST_UUID)
            .expect("build");
        let decoded = sequencer::EdgeSequencerRequest::decode(bytes.as_slice()).expect("decode");
        let update = match decoded.inner {
            Some(sequencer::edge_sequencer_request::Inner::SplineAnchorUpdate(u)) => u,
            other => panic!("expected SplineAnchorUpdate, got {:?}", other),
        };
        assert_eq!(update.order_id, 42);
        assert_eq!(update.symbol_id, 7);
        assert_eq!(update.anchor_price, 51_000.0);
    }

    #[test]
    fn test_validate_spline_regions_rejects_empty_sides() {
        let err = validate_spline_regions(&[], &[]).unwrap_err();
        assert!(matches!(err, GodarkError::InvalidInput(_)));
    }
    ...
    #[test]
    fn test_parse_spline_order_ack_roundtrip() {
        let ack = sequencer::SplineOrderAck {
            node_id: 1,
            sequence: 88,
            correlation_id: vec![],
            order_id: 1234,
        ...
        let resp = sequencer::NodeResponse {
            inner: Some(sequencer::node_response::Inner::SplineOrderAck(ack)),
        };
        let parsed = parse_spline_order_ack(&resp.encode_to_vec()).expect("parse");
        assert!(parsed.accepted);
        assert_eq!(parsed.order_id, "1234");
        assert_eq!(parsed.sequence, "88");
