...
pub(crate) fn make_order_error_from_code(numeric: Option<u32>) -> GodarkError {
    ...
    if let Ok(narrow) = u16::try_from(raw) {
        if let Some(entry) = find(narrow) {
            return GodarkError::Order {
                message: format!("{} ({}, code={})", entry.reason, entry.symbolic, entry.code),
...
mod tests {
    ...
    fn make_from_numeric_known() {
        let err = make_order_error_from_code(Some(2007));
        match err {
            GodarkError::Order {
                message,
                error_code,
    ...
    fn make_from_numeric_unknown() {
        let err = make_order_error_from_code(Some(7777));
        match err {
            GodarkError::Order {
                message,
                error_code,
    ...
    fn make_from_numeric_none() {
        let err = make_order_error_from_code(None);
        match err {
            GodarkError::Order {
                message,
                error_code,
