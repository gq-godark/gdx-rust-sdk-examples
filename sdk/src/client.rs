...
impl GodarkClient {
    ...
    /// Place one compressed spline liquidity curve on a symbol.
    ///
    /// Each maker may have at most one resting spline per symbol. Cancel via
    /// [`Self::cancel_order`] using the returned `order_id`.
    pub async fn place_spline_order(
        &mut self,
        symbol: &str,
        anchor_price: f64,
        ...
        leverage: u32,
        stp_mode: Option<i32>,
    ) -> Result<crate::types::SplineOrderAck, GodarkError> {
        self.ensure_ready()?;
        let symbol_id = self.resolve_symbol(symbol)?;
        let corr_id = Uuid::new_v4().into_bytes().to_vec();
        let uuid = self.current_user_uuid()?;
        let plaintext = proto_bridge::build_place_spline_order_proto(
            symbol_id,
            uuid.as_bytes(),
            anchor_price,
            bid_regions,
            ...
            &corr_id,
            leverage,
            stp_mode.unwrap_or(0),
        )?;
        let response = self
            .send_encrypted_command("spline_place", symbol_id, &plaintext, &corr_id)
            .await?;
        self.parse_spline_order_response(&response)
    }

    /// Refresh only the quote-center anchor on an existing resting spline.
    pub async fn update_spline_anchor(
        &mut self,
        symbol: &str,
        order_id: u64,
        anchor_price: f64,
        ...
        let corr_id = Uuid::new_v4().into_bytes().to_vec();
        let uuid = self.current_user_uuid()?;
        let plaintext = proto_bridge::build_spline_anchor_update_proto(
            order_id,
            symbol_id,
            uuid.as_bytes(),
            anchor_price,
        ...
        let response = self
            .send_encrypted_command("spline_anchor_update", symbol_id, &plaintext, &corr_id)
            .await?;
        self.parse_spline_order_response(&response)
    }
    ...
    async fn send_encrypted_command(
        ...
        let payload = if self.config.transport.use_docs_wire {
            let wire_op = match request_type {
                ...
                "spline_place" => "order.spline_place",
                "spline_anchor_update" => "order.spline_anchor_update",
                other => {
    ...
    fn parse_spline_order_response(
        &mut self,
        msg: &Value,
    ) -> Result<crate::types::SplineOrderAck, GodarkError> {
        let plaintext = self.decrypt_command_plaintext(msg, "spline_order_ack")?;
        proto_bridge::parse_spline_order_ack(&plaintext)
    }
