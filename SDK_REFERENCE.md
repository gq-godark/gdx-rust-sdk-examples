...
**Crate:** `godark` (vendored under `sdk/` in this repo)
...
- `Order { message: String, error_code: Option<String> }`
  ...
  `"MARGIN_INSUFFICIENT"`). See `quickstart.rs` for the match-and-print
  pattern.
- `Connection(String)`
- `Encryption(String)`
- `Timeout(String)`
...
| `examples/dotenv.rs` | Tiny shared helper (`load_dotenv` + `print_order_error`) |
...
To refresh `sdk/` from a sibling SDK checkout (internal use):

```bash
./scripts/refresh_sdk.sh /path/to/gdx-rust-sdk
```

The script preserves `sdk/src/generated/*.rs`, so consumers continue to
build without `protoc` or `prost-build`.
